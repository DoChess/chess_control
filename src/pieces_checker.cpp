...
bool PiecesChecker::look_for_pieces_in_the_way_rook(int x1, int y1, int x2, int y2){
  //Tem que ser menor ou igual pro que for igual
...
bool MotionValidation::look_for_pieces_at_destiny(int x, int y, int turn){
  if(
...
bool PiecesChecker::knight_checker(int x1, int y1, int x2, int y2){
  if(
      ((abs(x1 - x2) == 2 and abs(y1 - y2) == 1) or
       (abs(y1 - y2) == 2 and abs(x1 - x2) == 1)) and
       look_for_piece_at_destiny(x2,y2)
  ){
      return true;
  }else{
    return false;
...
bool PiecesChecker::king_checker(int x1, int y1, int x2, int y2){
  if(
     ((abs(x1 - x2) == 1 and abs(y1 - y2) == 1) or
      (abs(x1 - x2) == 1 and abs(y1 - y2) == 0) or
      (asb(y1 - y2) == 1 and abs(x1 - x2) == 0)) and
      look_for_pieces_at_destiny(x2, y2)
  ){
    return true;
  }else{
    return false;
...
bool PiecesChecker::rook_checker(int x1, int y1, int x2, int y2){
  ...
  if(x1 == x2 or y1 == y2){
    ...
    is_a_valid_movement = look_for_pieces_in_the_way(x1, y1, x2, y2);
  }else{
...
bool PiecesChecker::pawn_checker(int x1, int y1, int x2, int y2, turn){
...
bool PiecesChecker::bishop_checker(int x1, int y1, int x2, int y2){
  bool is_a_valid_movement = false;
